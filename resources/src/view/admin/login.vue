        ...
        <el-row type="flex" class="validate-wrapper fwrap">
            ...
            <el-col :xs="{span: 20, offset:2}" :sm="{span:20, offset:2}" :md="{span:4, offset:10}" class="content">
                ...
                <el-button :plain="true" type="success" class="register"
                    ...
                    <router-link to="/admin/register" tag="span">注册</router-link>
                </el-button>
            </el-col>
        </el-row>
    </div>
...
<script>
    ...
    export default{
        ...
        methods : {
           login () {
               this.$https.post('admin/login', this.form).then(
                   response => {
                        ...
                        if (data.status) {
                            ...
                            this.$store.commit('admin_login', data.data);
                            sessionStorage.setItem('admin_token', data.data.token);
                            window.sessionStorage.setItem('admin_user', JSON.stringify(data.data.user));
